...
export function advanceBasics(s,action){const n=s.step,g=s.g;if(action!==BASIC_STEPS[n]?.[0]||action==='done')return false;s.changes=[];
 ...
 if(n===9){s.selected=2;s.changes=['选中全部4颗2点','去2号桌'];}
...
export function createBasics({cinema,die,prefs,onExit}){
 const key='vegas-night-basics-v1';let state,root,busy=false;
 const save=()=>{try{localStorage.setItem(key,JSON.stringify({step:state.step}));}catch{}};
 function close(mode){save();root.remove();root=null;document.body.classList.remove('learning');onExit(mode);}
 const button=(a,label)=>`<button class="primary basic-hot" data-basic="${a}">${label}</button>`;
 function render(){const s=state,g=s.g,step=s.step,face=step>=9?2:6,t=g.tables[face-1],o=outcome(t),owners=banknoteOwners(t,o),done=step===11,current=BASIC_STEPS[step];
 ...
 root.dataset.step=String(step);if(!prefs().motion)root.classList.add('basic-still');
