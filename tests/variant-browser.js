async (page) => {
 ...
 const before=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v2')));
 for(let i=0;i<8;i++){
  await page.getByRole('button',{name:'掷骰子',exact:true}).click();
  await page.getByRole('button',{name:'跳过本次 · 下次重投'}).click();
 }
 const skipped=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v2')));
 ...
 if(!await page.getByRole('heading',{name:'霓虹街 · 先到先得'}).isVisible())throw Error('Rule dialog absent');
 await page.getByRole('button',{name:'回到牌桌',exact:true}).click();
 await page.reload();await page.getByRole('button',{name:'继续上次牌局'}).click();
 const restored=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v2')));
 ...
 let steps=0;
 while(steps++<700){
  const g=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v2')));
