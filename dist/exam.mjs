import {createGame,rollDice,placeDice,skipTurn,canSkip,useSkill,canSkill,outcome,previewTable,banknoteOwners,validSave,NAMES,COLORS,EFFECTS} from './core.mjs';
...
export function createExam(){
 let seed=1;const g=createGame(1,()=>((seed=(Math.imul(seed,1664525)+1013904223)>>>0)/4294967296),{hero:1,skills:true,rules:'classic'});
 ...
 g.move=6;g.turn=1;g.roll=[6,2];g.phase='choose';g.history=['毕业挑战 · 固定残局：牛仔还有2颗骰子，其他玩家已完成下注并用过主动技能。'];
...
export function examAdvice(s){
 ...
 if(s.last==='intro')return '最后一桌，交给你了。目标是6号的8万大钞：桌上你2颗、老板3颗、月兔1颗。你手中还有两颗骰子、一次狙击和两枚筹码；其他人已经完成下注并用过技能。练习骰子固定，打法由你选择。';
...
export function createFinalExam({cinema,die,prefs,onExit}){
 ...
 function act(a){if(busy||cinema.active)return;const g=state.g,previous=g.lastEvent,choice=a.choice,event=a.type==='skill'?{actor:1,target:choice.target,face:choice.face,zone:choice.zone,value:choice.zone==='roll'?g.roll[choice.index]:choice.face,before:choice.zone==='table'?g.tables[choice.face-1].counts[choice.target]:g.players[choice.target].left,human:true}:null;
 ...
 if(a.type==='roll'||a.type==='skill'){busy=true;if(event){event.after=event.before-1;event.label=g.history[0];}cinema.play(a.type==='roll'?'roll':'skill',event||{actor:1,values:[...g.roll]},done);}else done();
 }
 ...
 if(a==='fire'&&shot){const [zone,target,index]=shot.split(':');act({type:'skill',choice:{zone,target:Number(target),face:zone==='table'?Number(index):undefined,index:zone==='roll'?Number(index):undefined}});return;}
