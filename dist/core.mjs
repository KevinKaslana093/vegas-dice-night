...
export function createGame(humans=1,rng=Math.random,options={}){
  ...
  const g={v:4,hero,round:0,humans,rules,move:0,pending:null,lastEvent:null,players:NAMES.map(name=>({name,cash:0,notes:0,left:8,skips:0})),deck:deck(rng),tables:[],turn:0,roll:[],phase:'ready',history:[],settlements:[]};nextRound(g,rng);return g;
}
...
export function skipTurn(g){if(!canSkip(g))return false;const p=g.players[g.turn];p.skips++;g.lastEvent=null;log(g,`${p.name}跳过 · 留着${p.left}颗骰子，下次轮到时重投`);advance(g);return true;}
...
export function validSave(g){
  if(!g||g.v!==4||!Number.isInteger(g.hero)||g.hero<0||g.hero>3||![1,2,3,4].includes(g.round)||![1,2,4].includes(g.humans)||!['half','all','classic'].includes(g.rules)||!['ready','choose','minigame','settled','finished'].includes(g.phase))return false;
