...
export function createCinema({names,colors,skills,die,prefs}){
 ...
 function resultMarkup(e){
  ...
  if(e.actor===1)return `<div class="snipe-prop"><div class="crosshair"></div><div class="shot-die">${die(e.value||e.face||1)}</div><b class="shot-word">OUT</b></div><h3>一颗骰子 · 出局</h3><p>${safe(names[e.target])} · ${e.zone==='table'?e.face+'号赌场':e.zone==='roll'?'刚掷出的骰子':'手中骰子'} ${e.before} → ${e.after}</p>`;
 ...
 function cubes(values){const angles=[[0,0],[90,0],[0,-90],[0,90],[-90,0],[0,180]];return values.map((v,i)=>{const [x,y]=angles[v-1];return `<div class="cube-holder" style="--i:${i}"><div class="cube" data-result="${v}" style="--rx:${x}deg;--ry:${y}deg;--spin:${720+i*90}deg">${[1,2,3,4,5,6].map(n=>`<div class="cube-face cf-${n}">${die(n)}</div>`).join('')}</div></div>`;}).join('');}
