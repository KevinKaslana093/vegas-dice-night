async(parent)=>{
 ...
 await page.addInitScript(()=>{if(!sessionStorage.getItem('tutorial-qa')){localStorage.removeItem('vegas-night-tutorial-v2');localStorage.setItem('vegas-night-basics-v1',JSON.stringify({step:11}));localStorage.setItem('vegas-night-prefs-v2',JSON.stringify({fast:true,motion:false,sound:false}));sessionStorage.setItem('tutorial-qa','1');}});
 await page.reload();
 ...
 for(const size of [{width:1440,height:1050},{width:390,height:844},{width:360,height:740}]){
  ...
  await page.locator('#modal [data-action="tutorial"]').click();await page.locator('[data-basic="advanced"]').click();
  ...
  for(let step=0;step<24;step++){
   ...
   if(step===6&&size.width===1440){await page.locator('[data-lesson="exit"]').click();await page.reload();await page.locator('#modal [data-action="tutorial"]').click();await page.locator('[data-basic="advanced"]').click();await page.waitForFunction(()=>document.querySelector('.lesson-root')?.dataset.step==='6');}
