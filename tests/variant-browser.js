async (page) => {
 ...
 await page.locator('[data-pick="0"]').click();await page.locator('#edition').selectOption('normal');await page.getByLabel('今晚和谁玩？').selectOption('4');
 ...
 for(let i=0;i<8;i++){
  ...
  await page.getByRole('button',{name:'支付1筹码 · 跳过本次'}).click();
 }
 const skipped=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v5')));
 ...
 await page.screenshot({path:'output/playwright/v5-desktop.png',fullPage:true});
 for(const width of [360,390,768,1100,1500]){
  ...
  if(width===390)await page.screenshot({path:'output/playwright/v5-mobile.png',fullPage:true});
 }
 ...
 const restored=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v5')));
 ...
 await page.locator('#edition').selectOption('skills');await page.locator('[data-pick="2"]').click();if(await page.locator('[data-pick="2"]').getAttribute('aria-pressed')!=='true')throw Error('Role not selected');await page.getByLabel('今晚和谁玩？').selectOption('1');await page.locator('#ruleset').selectOption('all');
 ...
 while(steps++<1200){
  ...
  else if(g.turn===g.hero&&g.phase==='ready')await page.getByRole('button',{name:'掷骰子',exact:true}).click();
  else if(g.turn===g.hero&&g.phase==='choose'){
 ...
 if(steps>=1200)throw Error('No final result');if(errors.length)throw Error(errors.join(';'));
 return {paidSkip:true,sortedPrizes:true,restore:true,responsive:true,allSpecialGameComplete:true,steps,errors};
}
