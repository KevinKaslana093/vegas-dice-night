import {outcome,previewTable,banknoteOwners,MINIGAMES,EFFECTS,COLORS} from './core.mjs';
...
export function analyzeBet(g,face){
 ...
 return {face,name:before.name,effect:before.effect,count:g.roll.filter(n=>n===face).length,actor:g.turn,miniPending:MINIGAMES.includes(before.effect)&&!before.played[g.turn],
  players:g.players.map((_,i)=>({player:i,beforeDice:before.counts[i],afterDice:after.counts[i],bonus:before.bonus[i],beforeCash:value(a,i),afterCash:value(b,i),beforeTied:a.ties.includes(i),afterTied:b.ties.includes(i)})),
...
export function endgameInfo(g){
 ...
 return {active,total,kind:active.length===0?'mini':active.length===1?'solo':active.length===2?'duel':'last',unused:g.skills?g.players.map((p,i)=>({i,used:p.used})).filter(x=>x.i>0&&!x.used).map(x=>x.i):[]};
}
export function endgameMarkup(g){
 const e=endgameInfo(g);if(!e)return '';const who=i=>i===g.hero?'你':DECISION_NAMES[i],title=e.kind==='solo'?`只剩${who(e.active[0].i)}还能下注`:e.kind==='duel'?`${who(e.active[0].i)}与${who(e.active[1].i)}的最后争夺`:e.kind==='mini'?'骰子已投完 · 等待小游戏结果':`终盘 · 全桌只剩${e.total}颗骰子`;
