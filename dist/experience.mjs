...
export function skillCost(g,actor){const role=roleOf(g,actor),p=g.players[actor];return role===0?'被动生效，不需要确认。':`本轮限一次 · ${role===2?'代价：交出全部 '+p.chips+' 枚筹码。':role===6?'代价：随机失去一颗刚掷出的骰子，大骰也可能失去。':'不消耗筹码。'}${role===1?' 狙掉大骰减少2计数；也可能替对手解除撞数。':role===3?' 随机抽一张，不能指定票面。':role===5?' 重掷仍可能得到原来的点数。':''}`;}
...
export function publicBotChoice(g,difficulty='standard'){
 const me=g.turn,role=roleOf(g,me),cash=(o,i)=>o.awards.filter(a=>a.player===i).reduce((n,a)=>n+a.amount/10000,0),choices=[...new Set(g.roll)].map(face=>{
...
export function publicBotSkip(g,difficulty='standard'){
 const p=g.players[g.turn];if(g.phase!=='choose'||p.chips<1||p.skips>=2||difficulty==='relaxed')return false;
