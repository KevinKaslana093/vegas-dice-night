...
export function examAdvice(s){
 ...
 if(s.last==='hint')return ['','先看谁会撞数。相同计数的玩家一起失去领奖资格；钱要等最后一颗落下才发。','重点看6号桌：比较你与老板的计数。留下一颗再投，或者用狙击，都可能改变最后的排名。','参考初始局面的一种打法：先押6，再掷剩余骰子继续争6号。另一种是先狙掉老板在6号的一颗，再用6点超过他。顺序由你决定。'][s.hints];
...
export function createFinalExam({cinema,die,prefs,onExit}){
 ...
 function board(){const g=state.g;return `<section class="lesson-board exam-board" aria-label="毕业挑战赌场"><div class="lesson-orbit"></div><div class="lesson-center"><span>FINAL CHALLENGE</span><b>抢下6号大钞</b><small>固定残局 · 多种解法</small></div>${g.tables.map(t=>{const shown=state.selected===t.face?previewTable(g,t.face):t,o=outcome(shown),owners=banknoteOwners(shown,o);return `<article class="lesson-table lt-${t.face} ${t.face===6?'lesson-focus':''} ${state.selected===t.face?'lesson-selected':''}"><header><b aria-label="${t.face}号赌场">${die(t.face)}</b><span>${t.name}<small>总额 ${fmt(t.notes[0]+t.notes[1])}</small></span></header><div class="lesson-notes">${t.notes.map((n,j)=>`<span data-note="${j}">${fmt(n)}<small style="color:${owners[j]===null?'#a99db6':COLORS[owners[j]]}">${who(owners[j])}${owners[j]===null?'':examResult(state)==='playing'?'暂领':'获得'}</small></span>`).join('')}</div><div class="lesson-counts">${shown.counts.map((n,i)=>`<span data-player="${i}" style="color:${COLORS[i]}" class="${o.ties.includes(i)?'lesson-tied':''}">${['◆','●','■','▲'][i]} ${n?tableWeight(shown,i):'–'}${bigCount(shown,i)?' ◈':''}</span>`).join('')}</div><footer>${state.selected===t.face?'下注预览 · ':''}${EFFECTS[t.effect].name}</footer></article>`;}).join('')}</section>`;}
 ...
 root.innerHTML=`<div class="lesson-header"><div><span>GRADUATION / FREE PLAY</span><h1>最后一桌，你来赢</h1></div>${button('exit','退出挑战')}</div><div class="exam-goal"><b>目标：结算时拿到6号的8万大钞</b><span>${result==='won'?'✓ 已拿到':result==='lost'?'本次未完成':'进行中 · 非指定解法'}</span></div><div class="lesson-scroll"><div class="exam-stage">${endgameMarkup(g)}${board()}<section class="lesson-controls exam-controls">${controls()}</section><div class="lesson-claims">${claimMarkup(g.lastEvent,1,{settled:result!=='playing',compact:true})}</div></div></div><section class="lesson-coach exam-coach"><div class="lesson-portrait"><img src="${CINEMA_ASSETS.portraits[3]}" alt="月兔小姐"><span>你的领桌人</span></div><div class="lesson-dialogue"><div class="lesson-speaker">月兔小姐 <small>${result==='playing'?'YOUR CALL':'DEBRIEF'}</small></div><small class="exam-hint-level">${state.hints?'已展开 '+state.hints+' / 3 层提示':'可以先自己试试，提示按需展开'}</small><p aria-live="polite">${examAdvice(state)}</p><div class="lesson-reply">${result==='playing'?button('hint',state.hints>=3?'查看完整提示':state.hints===2?'提示3 · 一种解法':state.hints===1?'提示2 · 关键赌场':'提示1 · 思考方向'):button(result==='won'?'finish':'retry',result==='won'?'去正式牌桌':'重开这个残局',true,true)}</div></div></section>`;
 root.dataset.result=result;if(result==='won')document.dispatchEvent(new CustomEvent('vegas-training-complete',{detail:'exam'}));const scroller=root.querySelector('.lesson-scroll');scroller.scrollTop=oldScroll;
 ...
 if(a==='exit'||a==='finish'){close(a==='finish'?'select':undefined);return;}if(a==='retry'){feedback.clear();state=createExam();skillOpen=false;shot=null;save();render(true);return;}
