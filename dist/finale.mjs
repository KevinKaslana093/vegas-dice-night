import {NAMES,SKILLS,roleOf,outcome,previewTable,sum,winners} from './core.mjs';
export const HERO_LABELS=['短裤女士','筹码牛仔','赌场老板','兔女郎','冷面女荷官','潇洒魔术师'];
const HERO_LINES=['初次入局，也要多赢一手。','骰子落定之前，胜负未定。','欢迎来到我的主场。','别只盯着牌，也看看我。','偶然，需要一点精确。','看清了吗？再看一次。'];
export function heroSelection(role,art){
 const prev=(role+5)%6,next=(role+1)%6;
 return `<div class="hero-stage" data-hero-stage="${role}" aria-label="左右滑动选择英雄"><div class="hero-sweep"></div><div class="hero-ghost left" style="background-image:url('${art[prev]}')"></div><div class="hero-ghost right" style="background-image:url('${art[next]}')"></div><img draggable="false" class="hero-main" src="${art[role]}" alt="${HERO_LABELS[role]}全身立绘"><div class="hero-title"><small>CHOOSE YOUR HERO / 0${role+1}</small><h2>${HERO_LABELS[role]}</h2><p>「${HERO_LINES[role]}」</p><b>${SKILLS[role].name}</b><span>${SKILLS[role].text}</span></div><button class="hero-arrow hero-prev" data-hero-step="-1" aria-label="上一位英雄">‹</button><button class="hero-arrow hero-next" data-hero-step="1" aria-label="下一位英雄">›</button><small class="hero-nav-hint">← → 切换 · 手机左右滑动</small></div><div class="hero-roster" aria-label="六位可选英雄">${HERO_LABELS.map((name,i)=>`<button data-pick="${i}" aria-pressed="${role===i}" aria-label="选择${name}"><img draggable="false" src="${art[i]}" alt=""><small>${name}</small></button>`).join('')}</div>`;
}
export function forecastStanding(g,face=0){
 const cash=g.players.map(p=>p.cash),notes=g.players.map(p=>p.notes),done=['settled','finished'].includes(g.phase);
