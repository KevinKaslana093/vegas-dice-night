async(parent)=>{
 ...
 try{
  ...
  await p.goto('file:///C:/Users/34228/Documents/Codex/2026-09-11/w/outputs/拉斯维加斯-骰子之夜.html');await p.locator('.lobby-journey').waitFor();await p.locator('.lobby-journey').click();await p.locator('[data-contract="break"]').click();await p.locator('#app [data-action="roll"]').click();await p.waitForFunction(()=>!document.querySelector('#cinema[open]'));await p.locator('#app .dicebutton').first().click();await p.locator('.bet-quick').waitFor();await p.locator('#app [data-action="settings"]').click();await p.locator('[data-pref="music"]').check();await p.waitForFunction(()=>{const a=document.getElementById('game-music');return a.src.startsWith('blob:')&&!a.paused&&a.currentTime>.1&&!a.error;});
  await p.locator('#modal [data-action="close"]').click();await p.screenshot({path:'output/playwright/offline-v13-mobile.png'});
