async (page) => {
 ...
 const before=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v4')));
 ...
 const skipped=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v4')));
 ...
 await page.screenshot({path:'output/playwright/v4-desktop.png',fullPage:true});
 for(const width of [360,390,768,1100,1500]){
  ...
  if(width===390)await page.screenshot({path:'output/playwright/v4-mobile.png',fullPage:true});
 }
 ...
 const restored=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v4')));
 ...
 await page.locator('[data-pick="2"]').click();if(await page.locator('[data-pick="2"]').getAttribute('aria-pressed')!=='true')throw Error('Role not selected');await page.getByLabel('今晚和谁玩？').selectOption('1');await page.locator('#ruleset').selectOption('all');
 await page.getByRole('button',{name:'入座，开一局'}).click();await page.getByRole('button',{name:'明白了，回到牌桌'}).click();
 const chosen=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v4')));if(chosen.hero!==2||chosen.turn!==2||!await page.locator('.seat-2 .seat-top').innerText().then(s=>s.includes('你')))throw Error('Wrong controlled character');let steps=0;
 while(steps++<700){
  const g=await page.evaluate(()=>JSON.parse(localStorage.getItem('vegas-night-save-v4')));
