import {NAMES,COLORS,SKILLS,placedBig,roleOf,skillFor} from './core.mjs';
...
export function rivalLine(g,face,kind='invest'){
 ...
 if(hasBig)return ['大骰也来了，一起押。','这一颗，算两颗。','加重这一桌的分量。','小心，我这一颗顶两颗。'][role];
...
export function reactionMarkup(g){
 ...
 return `<section class="final-action" aria-label="最后行动"><small>LAST CALL · 奖金尚未发放</small><h2>${g.players[i].name}，最后一次选择</h2><p>发动「${skillFor(g,i).name}」，或放弃本轮最后行动。没有倒计时，确认后才轮到下一位。</p><div>${r.order.map((j,k)=>`<span class="${k===r.index?'current':''}">${k<r.index?'✓ ':''}${g.players[j].name}</span>`).join(' → ')}</div></section>`;
}
