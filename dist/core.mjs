import {BALANCED_SKILLS,balancedCan,balancedUse,balancedBot,validBalanced} from './balance.mjs';
import {ROYAL_MODULES,royalDeck,setupRoyal,royalClosed,royalRefresh,activateRoyal,prepareRoyalPayout,validRoyal} from './royale.mjs';
...
for(const m of ROYAL_MODULES)EFFECTS[m.id]={name:m.name,short:m.short,text:m.text};
...
export const skillFor=(g,i)=>(g.balance===2?BALANCED_SKILLS:SKILLS)[roleOf(g,i)];
export const initialSupply=g=>32+(g.skills&&g.players.some((_,i)=>roleOf(g,i)===0)?(g.balance===2?2:1):0)+(g.royale?.extra||0);
...
export function rematch(g){if(!g.random)return null;return createGame(g.humans,Math.random,{hero:g.hero,rules:g.rules,skills:g.skills,bigDice:g.bigDice,finalActions:g.finalActions,seed:g.random.seed,cast:g.players.map((_,i)=>roleOf(g,i)),balance:g.balance,modulePool:g.modulePool,chipScore:g.chipScore,royalSeen:g.royalSeen,practice:g.practice,roundLimit:g.roundLimit});}
export function createGame(humans=1,rng=Math.random,options={}){
  ...
  const g={v:5,bigDice:options.bigDice===true,finalActions:options.finalActions===true,reaction:null,turningPoint:null,skills:options.skills===true,hero,round:0,humans,rules,move:0,pending:null,lastEvent:null,players:cast.map(role=>({role,name:NAMES[role],cash:0,notes:0,wallet:[],chips:0,used:false,left:8,supply:8,lost:0,skips:0})),deck:[],tables:[],turn:0,roll:[],phase:'ready',history:[],settlements:[]};if(options.balance===2)g.balance=2;if(options.chipScore===true||options.modulePool==='royal')g.chipScore=true;if(options.modulePool==='royal'){g.modulePool='royal';g.royalSeen=Array.isArray(options.royalSeen)?options.royalSeen:[];}if(options.practice){g.practice=options.practice;g.roundLimit=1;}if(Number.isInteger(options.seed)&&options.seed>=0&&options.seed<=0xffffffff)g.random={seed:options.seed,counters:{}};g.deck=(g.modulePool==='royal'?royalDeck:deck)(gameRandom(g,'deck',rng));nextRound(g,rng);return g;
}
export function log(g,s){g.history.unshift(s);g.history=g.history.slice(0,48);}
export function nextRound(g,rng=Math.random){
  if(g.round>=(g.roundLimit||4))throw Error('Game finished');rng=gameRandom(g,'setup:'+(g.round+1),rng);g.round++;g.turn=(g.hero+g.round-1)%4;g.roll=[];g.phase='ready';g.settlements=[];g.move=0;g.pending=null;g.lastEvent=null;g.reaction=null;g.turningPoint=null;g.skillPending=null;
  g.players.forEach((p,i)=>{p.left=g.skills&&roleOf(g,i)===0?(g.balance===2?10:9):8;p.supply=p.left;p.lost=0;p.skips=0;p.used=false;p.chips+=2;if(g.bigDice){p.bigLeft=1;p.bigLost=0;}});
  ...
  if(g.modulePool==='royal')setupRoyal(g,rng);
  log(g,`第 ${g.round} 轮 · 每桌2张钞票${g.rules==='classic'?'':' · 特殊效果已重新抽取'} · ${g.players[g.turn].name}先手`);
}
export function rollDice(g,rng=Math.random){if(g.phase!=='ready'||g.skillPending)return false;rng=gameRandom(g,'dice:'+g.round+':'+g.turn,rng);g.roll=Array.from({length:g.players[g.turn].left},()=>1+Math.floor(rng()*6));if(g.practice&&g.move===0&&g.turn===g.hero)g.roll[0]=1;g.phase='choose';return true;}
export const bigCount=(t,i)=>t.big?.[i]||0;
export const tableWeight=(t,i)=>t.counts[i]+bigCount(t,i)+(t.bonus?.[i]||0);
export const isBigRoll=(g,index)=>!!g.bigDice&&index<(g.players[g.turn].bigLeft||0);
export const placedBig=(g,face)=>g.bigDice?g.roll.slice(0,g.players[g.turn].bigLeft||0).filter(n=>n===face).length:0;
export function outcome(t){
  ...
  if(t.neutral>0)eligible.push({n:t.neutral,i:4});
  ...
  return {ties:ties.filter(i=>i<4),rank:rank.filter(p=>p.i<4),awards:awards.filter(a=>a.player<4),unused:[...order.slice(awards.length),...awards.filter(a=>a.player===4).map(a=>a.amount)]};
}
...
export function advance(g){royalRefresh(g);g.roll=[];if(g.players.every(p=>p.left===0)){beginReaction(g);return;}do{g.turn=(g.turn+1)%4;}while(g.players[g.turn].left===0);g.phase='ready';}
export function placeDice(g,face,rng=Math.random){
  if(g.phase!=='choose'||g.skillPending||royalClosed(g,face)||!g.roll.includes(face))return false;rng=gameRandom(g,'mini:'+g.round+':'+g.turn+':'+face,rng);
  const rolledSnapshot=g.roll.map((value,index)=>({value,big:isBigRoll(g,index)}));
  const count=g.roll.filter(x=>x===face).length,actor=g.turn,t=g.tables[face-1];
  const before=outcome(t),snapshot=t.counts.map((_,i)=>tableWeight(t,i)),large=placedBig(g,face);g.move++;t.counts[actor]+=count;if(large){t.big[actor]+=large;g.players[actor].bigLeft-=large;}t.first[actor] ||= g.move;t.last[actor]=g.move;g.players[actor].left-=count;
  ...
  if(activateRoyal(g,face,actor,{placed:rolledSnapshot.filter(d=>d.value===face),remaining:rolledSnapshot.filter(d=>d.value!==face)}))return true;
...
export function canSkip(g){return !g.skillPending&&g.phase==='choose'&&g.players[g.turn].left>0&&(g.players[g.turn].chips>0||g.roll.every(face=>royalClosed(g,face)));}
export function skipTurn(g){if(!canSkip(g))return false;const p=g.players[g.turn];p.skips++;const forced=g.roll.every(face=>royalClosed(g,face));if(!forced)p.chips--;g.lastEvent=null;log(g,`${p.name}${forced?'仅掷出封闭桌点数，免费等待':'支付1枚筹码跳过'} · 留着${p.left}颗骰子，下次轮到时重投`);advance(g);return true;}
export function settle(g){if(g.skillPending||['settled','finished','reaction'].includes(g.phase))return;if(prepareRoyalPayout(g))return;g.reaction=null;g.settlements=g.tables.map(t=>{const o=outcome(t);o.awards.forEach(a=>{g.players[a.player].wallet.push(a.amount);syncMoney(g.players[a.player]);});g.deck.push(...o.unused);return o;});g.phase=g.round===(g.roundLimit||4)?'finished':'settled';}
export function syncMoney(p){p.cash=sum(p.wallet);p.notes=p.wallet.length;}
export function canSkill(g,actor){
 if(g.balance===2)return balancedCan(g,actor);if(g.skillPending||g.phase==='royale')return false;
...
export function useSkill(g,actor,choice,rng=Math.random){
  if(!canSkill(g,actor)||!choice)return false;if(g.balance===2&&roleOf(g,actor)!==1)return balancedUse(g,actor,choice,rng);const {target,zone,face,index,big=false}=choice;
  ...
  if(role>=4){
   ...
   if(role===4){if(!Number.isInteger(index)||index<0||index>=g.roll.length||!Number.isInteger(choice.value)||choice.value<1||choice.value>6||g.roll[index]===choice.value)return false;const old=g.roll[index];g.roll[index]=choice.value;detail=`将自己的${isBigRoll(g,index)?'大骰':'普通骰'}从${old}点改为${choice.value}点`;}
    ...
    if(zone==='table'){
      if(!Number.isInteger(face)||face<1||face>6||royalClosed(g,face))return false;const t=g.tables[face-1];if(!t.counts[target])return false;
      ...
      const large=isBigRoll(g,index);const value=g.roll.splice(index,1)[0];if(large){other.bigLeft--;other.bigLost++;}other.left--;other.lost++;g.lastEvent=null;detail=`狙掉${other.name}刚掷出的${value}点${large?'大骰':'普通骰'}`;
    }else if(zone==='hand'){
      if(other.left<1||(g.phase==='choose'&&target===g.turn))return false;
      if(big?!other.bigLeft:other.left<=(other.bigLeft||0))return false;other.left--;other.lost++;if(big){other.bigLeft--;other.bigLost++;}g.lastEvent=null;detail=`狙掉${other.name}手中的一颗${big?'大骰':'普通骰'}`;
    }else return false;
  }else if(role===2){
  ...
  royalRefresh(g);p.used=true;log(g,`${p.name}发动${SKILLS[role].name} · ${detail}`);
...
export function botSkill(g,actor,rng=Math.random){
 if(!canSkill(g,actor))return null;if(g.balance===2&&roleOf(g,actor)!==1)return balancedBot(g,actor);const role=roleOf(g,actor);
 if(role===6){
 ...
 for(const t of g.tables.filter(t=>!royalClosed(g,t.face)))for(let target=0;target<4;target++)for(const big of role===1?[false,true]:[false]){
...
export const scoreFor=(g,i)=>g.players[i].cash+(g.chipScore?g.players[i].chips*10000:0);
export function winners(g){const rank=g.players.map((p,i)=>({...p,i,score:scoreFor(g,i)})).sort((a,b)=>b.score-a.score||b.notes-a.notes);return rank.filter(p=>p.score===rank[0].score&&p.notes===rank[0].notes).map(p=>p.i);}
export function botChoice(g,rng=Math.random){
  ...
  const choices=[...new Set(g.roll)].filter(face=>!royalClosed(g,face)).map(face=>{const n=g.roll.filter(x=>x===face).length,before=outcome(g.tables[face-1]),after=outcome(previewTable(g,face));const t=g.tables[face-1],spread=g.players[me].left>3&&!t.counts[me],trade=role===2&&canSkill(g,me)&&g.players[me].left>3&&t.counts[me]===0;const style=role===0?(spread?.75:0):role===1?(after.ties.includes(me)?-.4:0):role===2?(trade?.45:0):-.1*n;return {face,score:(own(after)-own(before))/10000+(rivals(before)-rivals(after))/50000-n*.15+style+rng()*.8};}).sort((a,b)=>b.score-a.score);
...
export function botShouldSkip(g,rng=Math.random){
  ...
  const face=botChoice(g,()=>.5);if(!face)return true;const before=outcome(g.tables[face-1]),after=outcome(previewTable(g,face));
...
export function validSave(g){
  if(!g||g.v!==5||typeof g.skills!=='boolean'||!Number.isInteger(g.hero)||g.hero<0||g.hero>3||![1,2,3,4].includes(g.round)||![1,2,4].includes(g.humans)||!['half','all','classic'].includes(g.rules)||!['ready','choose','minigame','reaction','royale','settled','finished'].includes(g.phase))return false;
  if(g.bigDice!==undefined&&typeof g.bigDice!=='boolean'||g.finalActions!==undefined&&typeof g.finalActions!=='boolean')return false;
  const int=(n,max)=>Number.isInteger(n)&&n>=0&&n<=max,note=n=>int(n,g.royale?100000:90000)&&n>=10000&&n%10000===0;
  if(g.random!==undefined&&(!g.random||!int(g.random.seed,0xffffffff)||!g.random.counters||Array.isArray(g.random.counters)||typeof g.random.counters!=='object'||Object.keys(g.random.counters).length>200||!Object.entries(g.random.counters).every(([k,v])=>/^(deck|setup:[1-4]|dice:[1-4]:[0-3]|mini:[1-4]:[0-3]:[1-6]|steal:[1-4]:[0-3]|decision:[1-4]:[0-3]|skip:[1-4]:[0-3]|magic:[1-4]:[0-3]|overtime:[1-4]:[0-3]|royal:[1-4]|royalbot:[1-4])$/.test(k)&&int(v,10000))))return false;
  if(!int(g.turn,3)||!int(g.move,g.royale?2000:100)||!Array.isArray(g.players)||g.players.length!==4||!Array.isArray(g.tables)||g.tables.length!==6)return false;
  if(!g.players.every(p=>(p.role===undefined||int(p.role,NAMES.length-1))&&typeof p.name==='string'&&p.name.length<30&&int(p.left,g.balance===2||g.royale?40:9)&&int(p.chips,g.royale?500:32)&&typeof p.used==='boolean'&&int(p.supply,g.balance===2||g.royale?40:33)&&int(p.lost,8)&&Array.isArray(p.wallet)&&p.wallet.every(note)&&p.cash===sum(p.wallet)&&p.notes===p.wallet.length&&int(p.cash,g.royale?20000000:3000000)&&int(p.notes,g.royale?500:54)&&int(p.skips,Number.MAX_SAFE_INTEGER)))return false;
  ...
  if(!g.tables.every((t,i)=>t.face===i+1&&t.name===CASINOS[i]&&Object.hasOwn(EFFECTS,t.effect)&&Array.isArray(t.notes)&&t.notes.length===2&&t.notes.every(note)&&['counts','first','last'].every(key=>Array.isArray(t[key])&&t[key].length===4&&t[key].every(n=>int(n,key==='counts'?40:g.royale?2000:100)))))return false;
  if(g.bigDice){if(!g.players.every(p=>int(p.bigLeft,g.balance===2||g.royale?4:1)&&p.bigLeft<=p.left&&int(p.bigLost,4)&&p.bigLost<=p.lost)||!g.tables.every(t=>Array.isArray(t.big)&&t.big.length===4&&t.big.every((n,i)=>int(n,4)&&n<=t.counts[i]))||sum(g.players.map(p=>p.bigLeft+p.bigLost))+sum(g.tables.flatMap(t=>t.big))+(g.royale?.held.filter(d=>d.big).length||0)!==4)return false;}else if(g.players.some(p=>p.bigLeft||p.bigLost)||g.tables.some(t=>t.big?.some(n=>n!==0)))return false;
  if(g.phase==='reaction'){const r=g.reaction;if(!g.finalActions||!g.skills||!g.players.every(p=>p.left===0)||!r||!Array.isArray(r.order)||r.order.length>3||new Set(r.order).size!==r.order.length||!r.order.every(i=>Number.isInteger(i)&&i>=0&&i<4)||!int(r.index,r.order.length-1)||r.order[r.index]!==g.turn||(!hasSkillTarget(g,g.turn)&&!g.skillPending)||!Array.isArray(g.settlements)||g.settlements.length)return false;}else if(g.reaction!=null)return false;
  if(!g.players.every((p,i)=>sum(g.tables.map(t=>t.counts[i]))+p.left+p.lost+(g.royale?.held.filter(d=>d.owner===i).length||0)===p.supply))return false;
  ...
  if((g.phase==='finished'&&g.round!==(g.roundLimit||4))||(g.phase==='settled'&&g.round===(g.roundLimit||4)))return false;if(!validBalanced(g)||!validRoyal(g))return false;return true;
}

export function feedback(g,t,actor,before,action='小游戏加成',beforeCounts=null){
...
function hasSkillTarget(g,actor){
 if(g.balance===2){if(g.players[actor].used||!g.skills||roleOf(g,actor)===0||roleOf(g,actor)>=4)return false;const copy={...g,phase:'ready',skillPending:null};return balancedCan(copy,actor);}
...
export function passReaction(g,actor=g.turn){
 if(g.skillPending||g.phase!=='reaction'||actor!==g.turn)return false;
